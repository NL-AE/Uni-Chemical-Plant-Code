  ...
  // Loop  --------------------------------------------------------------
    void loop() {
      ...
      // Check Serial
        char SerData[12];  //8 pin states, 3 for set temp, 1 for null pointer
  
        while (Serial.available() > 0) {                                              //When serial available
           int SerialLen = Serial.readBytesUntil('\n', SerData, sizeof(SerData) - 1);   // i = length of string
           SerData[SerialLen] = '\0';                                                   // null pointer when he gets dereferanced idk havent done it     
           
           if(SerialLen>10){          // Only use if data is full
              for(int i=0; i<8; i++){   // For first 8 pins add to array
                pinArray[i]=SerData[i]-48;
              }
              int SetTemp = 100*(SerData[8]-48)+ 10*(SerData[9]-48)+SerData[10]-48; // Extract set temperature
           }
         }
